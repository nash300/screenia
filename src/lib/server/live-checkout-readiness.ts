...
import {
  cookieTrackingGovernanceReadiness,
  csrfProtectionReadiness,
  dataProcessorRegisterReadiness,
  projectFilePath,
  securityHeaderReadiness,
  sensitiveNoStorePolicyReadiness,
  serviceWorkerCacheSafetyReadiness,
  stripeFinancialRiskWebhookReadiness,
} from "./live-checkout-security-readiness";
...
function finalLegalVersionReadiness(): CheckResult {
  const prelaunchVersions = [
    /prelaunch|utkast/i.test(CURRENT_TERMS_VERSION) ? "terms" : null,
    /prelaunch|utkast/i.test(CURRENT_PRIVACY_VERSION) ? "privacy" : null,
  ].filter(Boolean);
...
async function pricingReady(supabaseAdmin: SupabaseClient): Promise<CheckResult> {
  const { data, error } = await supabaseAdmin
    .from("pricing_plans")
    .select(
      "id, code, is_active, setup_fee_sek, setup_included_screens, additional_setup_fee_sek, hardware_fee_sek, shipping_fee_sek, shipping_included_devices, additional_shipping_fee_sek, monthly_fee_sek, currency, tax_behavior, stripe_setup_price_id, stripe_additional_setup_price_id, stripe_hardware_price_id, stripe_shipping_price_id, stripe_additional_shipping_price_id, stripe_monthly_price_id",
    );
  ...
  const issues = activePlans.flatMap((plan) => {
    ...
    if (Number(plan.shipping_fee_sek || 0) !== 99) {
