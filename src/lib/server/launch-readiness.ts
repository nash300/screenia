...
export const LIVE_PAYMENT_CONFIRMATION_FLAGS = [
  {
    key: "SCREENIA_LIVE_PAYMENTS_ENABLED",
    label: "Live payments enabled",
    detail: "Final switch that allows live Stripe checkout sessions.",
  },
  {
  ...
  },
  {
    ...
    label: "Legal review",
    detail: "Terms, privacy, cookie, subscription, support, and refund policies are reviewed for live customers.",
  },
  {
    key: "SCREENIA_LIVE_WEBHOOK_VERIFIED",
    label: "Live webhook verified",
    detail: "Stripe live webhook endpoint and event handling have been verified.",
  },
  {
    key: "SCREENIA_SUPABASE_AUTH_EMAIL_VERIFIED",
    label: "Supabase Auth email verified",
    detail:
      "Supabase Auth password setup and reset emails have been tested with the production sender/domain.",
  },
] as const;

export function isEnabledFlag(name: string) {
  return process.env[name] === "true";
}

export function getLivePaymentConfirmations() {
  return LIVE_PAYMENT_CONFIRMATION_FLAGS.map((flag) => ({
    ...flag,
    confirmed: isEnabledFlag(flag.key),
  }));
}

export function getMissingLivePaymentConfirmations() {
  return getLivePaymentConfirmations().filter((flag) => !flag.confirmed);
}

export function livePaymentsAreFullyConfirmed() {
  return getMissingLivePaymentConfirmations().length === 0;
}

function getApplicationUrlReadiness() {
...
function getCompanyIdentityReadiness() {
  const missing = [
...
function getFinalLegalVersionReadiness() {
...
export function hasTextQualityCheckConfigured() {
...
export function hasTransactionalEmailWorkflow() {
  ...
  const sourceIssues = emailRouteFiles.flatMap((file) => {
  ...
  return {
    ...
    details:
      sourceIssues.length === 0
        ? "Onboarding, quote, and request-confirmation emails use the shared transactional email sender with audited delivery state and admin failure notifications."
        : sourceIssues.join(" | "),
  };
}

export async function hasResendDeliveryEventWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  const webhookSource = readFileSync(
    projectFilePath("src/app/api/resend/webhook/route.ts"),
    "utf8",
  );
  const adminRouteSource = readFileSync(
    projectFilePath("src/app/api/admin/email-events/route.ts"),
    "utf8",
  );
  const adminPageSource = readFileSync(
    projectFilePath("src/app/admin/email-events/page.tsx"),
    "utf8",
  );
  const proxySource = readFileSync(projectFilePath("src/proxy.ts"), "utf8");
  const migrationSource = readFileSync(
    projectFilePath("supabase/migrations/202607120015_resend_delivery_events.sql"),
    "utf8",
  );
  const sourceIssues = [
    !webhookSource.includes("new Webhook(webhookSecret).verify")
      ? "Resend webhook does not verify Svix signatures"
      : null,
    !webhookSource.includes("svix-id") ||
    !webhookSource.includes("svix-timestamp") ||
    !webhookSource.includes("svix-signature")
      ? "Resend webhook does not use required Svix headers"
      : null,
    !webhookSource.includes("resend_delivery_event_received")
      ? "Resend webhook receipt is not audited"
      : null,
    !webhookSource.includes("resend_delivery_action_required")
      ? "Resend bounce/complaint events do not create admin notifications"
      : null,
    ...
    !proxySource.includes("/api/resend/webhook")
      ? "Resend webhook is not exempt from cross-origin webhook blocking"
      : null,
    !migrationSource.includes("svix_id") ||
    !migrationSource.includes("event_status")
      ? "Resend delivery event migration does not track duplicate event ids and statuses"
      : null,
  ].filter(Boolean);
  ...
  const [eventResult, auditResult, notificationResult] = await Promise.all([
    supabaseAdmin
  ...
  return {
    ...
    details: errors.length
      ...
      : "Resend delivery webhooks are signature verified, stored in a duplicate-safe event ledger, audited, visible to admins, and notify admins about bounces/complaints/failures.",
  };
}

export function hasCustomerSupportReplyWorkflow() {
  ...
  const sourceIssues = [
    ...
    !adminMessageRouteSource.includes("customer_support_reply_sent")
      ? "admin support replies are not audited"
      : null,
    ...
    !adminMessageRouteSource.includes("rollbackSupportReply")
      ? "admin support replies do not roll back and notify admins when reply audit storage fails"
      : null,
    !adminMessageRouteSource.includes("customer_message_admin_update")
      ? "admin support message review updates are not audited"
      : null,
    ...
    !adminMessageRouteSource.includes("Customer support reply email audit error")
      ? "admin support reply email delivery state is not audited"
      : null,
    ...
    !adminMessageRouteSource.includes("priority: \"urgent\"")
      ? "admin support reply email failures do not notify admins"
      : null,
    ...
    !adminCustomerPageSource.includes("Customer-visible reply")
      ? "admin customer page does not expose a customer-visible reply composer"
      : null,
    !adminCustomerPageSource.includes("sendCustomerMessageReply")
      ? "admin customer page is missing the support reply action"
      : null,
    !adminCustomerPageSource.includes(
  ...
  return {
    ...
    details:
      sourceIssues.length === 0
        ? "Admins can send customer-visible support replies that are threaded, audited, emailed through the shared sender, and visible in the customer account history."
        : sourceIssues.join(" | "),
  };
}

export async function hasCustomerSupportTicketIntakeWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const [messageResult, fileResult, auditResult, notificationResult] =
    await Promise.all([
      supabaseAdmin
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Customer support ticket intake validates text and attachments, audits submissions, notifies admins, and escalates privacy requests into the data subject request register.",
  };
}

export function hasBillingPortalWorkflow() {
...
export function hasCustomerConsentManagementWorkflow() {
...
export function hasPasswordResetAbuseProtection() {
  ...
  const sourceIssues = [
    ...
    !routeSource.includes("PASSWORD_RESET_GENERIC_MESSAGE")
      ? "password reset route does not use the generic response message"
      : null,
    ...
    !routeSource.includes("password_reset_email_notification_failed")
      ...
      : null,
    !routeSource.includes("resetPasswordForEmail")
      ? "password reset route does not request Supabase reset emails"
      : null,
  ].filter(Boolean);
  ...
  return {
    ok,
    details: ok
      ? `Password reset requests are rate limited to ${PASSWORD_RESET_EMAIL_LIMIT} per email and ${PASSWORD_RESET_IP_LIMIT} per IP per hour with a generic response.`
      : [
          !policyOk
...
export function hasLoginAttemptProtection() {
  const routeSource = readFileSync(
  ...
  return {
    ok,
    details: ok
      ? `Email/password login attempts are rate limited to ${LOGIN_ATTEMPT_EMAIL_LIMIT} per email and ${LOGIN_ATTEMPT_IP_LIMIT} per IP per 15 minutes.`
      : [
...
export async function hasSubscriptionOperationsMigration(
  supabaseAdmin: SupabaseClient,
) {
...
export async function hasSwedishRegistrationNumberValidation(
  supabaseAdmin: SupabaseClient,
) {
  const validatorOk =
    isValidSwedishRegistrationNumber("556016-0680") &&
    isValidSwedishRegistrationNumber("165560160680") &&
    !isValidSwedishRegistrationNumber("556016-0681") &&
    !isValidSwedishRegistrationNumber("123");
  ...
  const recordsNeedingValidNumber = (data || []).filter((customer) => {
  ...
  const invalidCount = recordsNeedingValidNumber.filter(
    (customer) =>
  ...
  return {
    ...
    details:
      invalidCount === 0
        ? "Swedish organisation-number validation is active, and current post-onboarding/payment customer records pass it."
        : `${invalidCount} post-onboarding/payment customer record(s) have missing or invalid Swedish organisation numbers.`,
  };
}

export async function hasPricingConfigurationReadiness(
  supabaseAdmin: SupabaseClient,
) {
  const { data, error } = await supabaseAdmin
    .from("pricing_plans")
  ...
  if (activePlans.length === 0) {
    return {
      ok: false,
      details: "No active pricing plans are available for quotes and checkout.",
    };
  }
  ...
  const issues = activePlans.flatMap((plan) => {
    ...
    const currency = String(plan.currency || "sek").toLowerCase();
    const hardwareFee = Number(plan.hardware_fee_sek || 0);
    const shippingFee = Number(plan.shipping_fee_sek || 0);

    if (currency !== "sek") planIssues.push(`${label}: currency is not SEK`);
    ...
    if (hardwareFee < 0 || shippingFee < 0) {
      planIssues.push(`${label}: one-time fees cannot be negative`);
    }
    if (Number(plan.trial_days || 0) < 0 || Number(plan.trial_days || 0) > 365) {
      planIssues.push(`${label}: trial days are outside launch bounds`);
    }
    if (
      Number(plan.binding_months || 0) < 0 ||
      Number(plan.binding_months || 0) > 365
    ) {
      planIssues.push(`${label}: binding months are outside launch bounds`);
    }
...
export async function hasCurrentLegalDocuments(supabaseAdmin: SupabaseClient) {
  const { data, error } = await supabaseAdmin
    .from("legal_documents")
    .select("document_type, version, status, pdf_url")
  ...
  const hasTerms = data?.some(
    (item) =>
      item.document_type === "terms" &&
      item.version === CURRENT_TERMS_VERSION &&
      item.pdf_url === CURRENT_TERMS_DOCUMENT.pdfUrl,
  );
  const hasPrivacy = data?.some(
    (item) =>
      item.document_type === "privacy" &&
      item.version === CURRENT_PRIVACY_VERSION &&
      item.pdf_url === CURRENT_PRIVACY_DOCUMENT.pdfUrl,
  );
  const pdfChecks = [
    {
      label: "terms PDF",
      path: CURRENT_TERMS_DOCUMENT.pdfUrl,
    },
    {
      label: "privacy PDF",
      path: CURRENT_PRIVACY_DOCUMENT.pdfUrl,
    },
  ].map((pdf) => {
    const publicPath = projectFilePath("public", pdf.path.replace(/^\//u, ""));
    const exists = existsSync(publicPath);
    const size = exists ? statSync(publicPath).size : 0;
    const header = exists ? readFileSync(publicPath, { encoding: "utf8", flag: "r" }).slice(0, 5) : "";

    return {
      ...pdf,
      ok: exists && size > 1000 && header === "%PDF-",
    };
  });
  const missingPdfs = pdfChecks.filter((pdf) => !pdf.ok);

  return {
    ok: Boolean(hasTerms && hasPrivacy && missingPdfs.length === 0),
    details: `terms=${hasTerms ? "yes" : "no"}, privacy=${
      hasPrivacy ? "yes" : "no"
    }, pdfs=${missingPdfs.length === 0 ? "yes" : missingPdfs.map((pdf) => pdf.label).join(", ")}`,
  };
}

export function hasDataProcessorRegister() {
  ...
  return {
    ...
    details:
      sourceIssues.length === 0
        ? "Data processor register exists and the public privacy policy discloses Supabase, Stripe, Resend, Vercel, and Loopia before live launch."
        : sourceIssues.join(" | "),
  };
}

export async function hasProcessorComplianceReviewWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const [processorResult, auditResult, notificationResult] = await Promise.all([
    supabaseAdmin
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Admins can record processor DPA, security, account-owner, region, evidence, and next-review details with audit history and admin notifications for incomplete provider evidence.",
  };
}

export async function hasLegalChangeNoticeWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const [noticeResult, auditResult, notificationResult] = await Promise.all([
    supabaseAdmin
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Admins can track legal/policy version changes, customer notice requirements, re-acceptance requirements, sent evidence, reasons, audit history, and follow-up notifications.",
  };
}

export async function hasLegalBeforePaymentWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const onboardingPageSource = readFileSync(
    projectFilePath("src/app/onboarding/[token]/page.tsx"),
    "utf8",
  );
  const sourceIssues = [
    !onboardingRouteSource.includes("recordLegalAgreement")
      ? "onboarding route does not store legal agreement snapshots"
      : null,
    !onboardingRouteSource.includes("recordConsent")
      ? "onboarding route does not store consent records"
      : null,
    !onboardingRouteSource.includes("throwOnError: true") ||
    !onboardingRouteSource.includes("Required legal evidence was not stored")
      ? "onboarding route does not block when required legal evidence fails to store"
      : null,
    !onboardingRouteSource.includes("terms_accepted_at") ||
    !onboardingRouteSource.includes("privacy_accepted_at")
      ? "onboarding route does not stamp accepted terms/privacy timestamps"
      : null,
    ...
    !onboardingRouteSource.includes("Onboarding profile evidence was not stored")
      ? "onboarding route does not fail visibly and roll back when optional consent or completion audit evidence fails"
      : null,
    !checkoutRouteSource.includes("hasRequiredLegalEvidence")
      ? "checkout route does not verify legal evidence before payment"
      : null,
    !checkoutRouteSource.includes("CURRENT_TERMS_VERSION") ||
    !checkoutRouteSource.includes("CURRENT_PRIVACY_VERSION")
      ? "checkout route does not require current legal document versions"
      : null,
    !checkoutRouteSource.includes("customer_legal_agreements") ||
    !checkoutRouteSource.includes("consent_records")
      ? "checkout route does not check both consent and legal agreement evidence"
      : null,
    !checkoutRouteSource.includes("Kunden m") ||
    !checkoutRouteSource.includes("integritetspolicy")
      ? "checkout route does not return a customer-safe legal evidence error"
      : null,
    ...
    !checkoutRouteSource.includes("priority: \"urgent\"")
      ? "checkout route does not audit and urgently notify admins when Stripe checkout creation fails"
      : null,
    !checkoutRouteSource.includes("Stripe checkout failure evidence was not stored")
      ? "checkout route does not fail visibly when checkout failure evidence cannot be stored"
      : null,
    ...
    !checkoutRouteSource.includes("checkout_session")
      ? "checkout route does not audit and urgently notify admins when Stripe succeeds but local checkout sync fails"
      : null,
    ...
    !checkoutRouteSource.includes("Stripe customer local sync failure evidence was not stored")
      ? "checkout route does not fail visibly when local-sync failure evidence cannot be stored"
      : null,
    ...
    !checkoutRouteSource.includes("{ throwOnError: true }")
      ? "checkout route can return a Stripe URL without stored checkout-start audit evidence"
      : null,
    !onboardingPageSource.includes("acceptedTerms") ||
    !onboardingPageSource.includes("acceptedPrivacy")
      ? "onboarding page does not collect terms and privacy acceptance"
      : null,
  ].filter(Boolean);
  ...
  const [consentResult, agreementResult, legalDocumentResult, auditResult] =
    await Promise.all([
      supabaseAdmin
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Onboarding stores required current terms/privacy consent and legal agreement snapshots before checkout, and checkout blocks payment when evidence is missing.",
  };
}

export async function hasPrivateVideoBucket(supabaseAdmin: SupabaseClient) {
  const { data, error } = await supabaseAdmin.storage.getBucket("videos");
  ...
  if (!data) {
    return { ok: false, details: "videos bucket is missing" };
  }
...
export async function hasPrivateSensitiveStorageBuckets(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const sourceChecks = [
    {
      label: "display material API",
      source: readFileSync(
        projectFilePath("src/app/api/admin/customer-assets/route.ts"),
        "utf8",
      ),
      bucket: "customer-display-assets",
    },
    {
      label: "support message API",
      source: readFileSync(
        projectFilePath("src/app/api/admin/customer-messages/route.ts"),
        "utf8",
      ),
      bucket: "customer-message-files",
    },
    {
      label: "display playlist API",
      source: readFileSync(
  ...
  const sourceIssues = sourceChecks.flatMap((checkItem) =>
    checkItem.source.includes("createSignedUrl")
      ? []
      : [`${checkItem.label} does not use signed URLs for ${checkItem.bucket}`],
  );
  const displayPlaylistSource = sourceChecks.find(
    (checkItem) => checkItem.label === "display playlist API",
  )?.source || "";

  if (displayPlaylistSource.includes('row.src?.startsWith("http")')) {
    sourceIssues.push(
      "display playlist API still allows direct public HTTP video URLs",
    );
  }
  if (!displayPlaylistSource.includes("hasDisplayEntitlement")) {
    sourceIssues.push("display playlist API does not check billing entitlement");
  }
  if (
  ...
  const migrationSource = readFileSync(
    projectFilePath(
      "supabase/migrations/202607120007_private_sensitive_customer_storage.sql",
    ),
    "utf8",
  );

  if (!migrationSource.includes("customer-display-assets")) {
    sourceIssues.push("sensitive storage migration does not cover display assets");
  }
  if (!migrationSource.includes("customer-message-files")) {
    sourceIssues.push("sensitive storage migration does not cover support attachments");
  }
  ...
  return {
    ...
    details:
      allIssues.length === 0
        ? "Videos, customer display material, and support attachments are in private buckets and are served through short-lived signed URLs."
        : allIssues.join(" | "),
  };
}

export async function hasStripeWebhookEventLedger(
  supabaseAdmin: SupabaseClient,
) {
  ...
  return {
    ...
    details: error
      ? error.message
      : "Stripe webhook event ledger is available for duplicate-event protection.",
  };
}

export function hasStripeFinancialRiskWebhookWorkflow() {
  const webhookSource = readFileSync(
    projectFilePath("src/app/api/stripe/webhook/route.ts"),
    "utf8",
  );
  const migrationSource = readFileSync(
    projectFilePath(
      "supabase/migrations/202607120008_payment_dispute_readiness.sql",
    ),
    "utf8",
  );
  const sourceIssues = [
    ...
    !webhookSource.includes("charge.refunded")
      ? "Stripe external charge-refunded webhook is not handled"
      : null,
    !webhookSource.includes("refund.updated")
      ? "Stripe refund-updated webhook is not handled"
      : null,
    !webhookSource.includes("payment_disputed")
      ? "Disputed payments do not block display entitlement"
      : null,
    ...
    !webhookSource.includes("payment_refunded_externally")
      ? "External Stripe refunds are not audited distinctly"
      : null,
    ...
    !webhookSource.includes("Stripe refund evidence storage error")
      ...
      : null,
    !webhookSource.includes("createAdminNotification")
      ? "Stripe financial risk events do not notify admins"
      : null,
    !migrationSource.includes("payment_disputed")
      ? "Payment dispute entitlement migration is missing"
      : null,
  ].filter(Boolean);
  ...
  return {
    ...
    details:
      sourceIssues.length === 0
        ? "Stripe disputes, won disputes, and external refunds are audited, notify admins, and block display access when payment is at risk."
        : sourceIssues.join(" | "),
  };
}

export async function hasRefundAndProductionBoundaryWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const sourceIssues = [
    ...
    !productionRouteSource.includes("Layout start rollback failure notification error")
      ...
      : null,
    !customerPageSource.includes("refundFirstPayment") ||
    !customerPageSource.includes("startLayoutWork") ||
    !customerPageSource.includes("Setup fee refund boundary")
      ? "admin customer page does not expose refund/layout boundary controls"
      : null,
  ].filter(Boolean);
  ...
  const [customerResult, subscriptionResult, auditResult, notificationResult] =
    await Promise.all([
      supabaseAdmin
        .from("customers")
        .select(
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Refund-before-layout and layout-start fee-lock boundaries require admin reasons, audit events, Stripe/admin metadata, active entitlement before production, and refund display blocking.",
  };
}

export async function hasOptInConsentDefaults(supabaseAdmin: SupabaseClient) {
  const { data, error } = await supabaseAdmin.rpc(
    "screenia_consent_defaults_are_opt_in",
  );
  ...
  return {
    ...
    details:
      data === true
        ? "Optional analytics, marketing, and remote-support consent default to false, and draft customers do not have analytics pre-enabled."
...
export function hasCookieTrackingGovernance() {
  const files = collectTrackingSourceFiles();
  const findings = files.flatMap((file) => {
    const source = readFileSync(projectFilePath(file), "utf8");

    return NON_ESSENTIAL_TRACKING_PATTERNS.flatMap((trackingPattern) =>
      trackingPattern.pattern.test(source)
        ? [`${trackingPattern.label} in ${file}`]
        : [],
    );
  });
  const cookiePolicySource = readFileSync(
    projectFilePath("src/app/cookie-policy/page.tsx"),
    "utf8",
  );
  const consentMechanismPresent = files.some((file) => {
    const source = readFileSync(projectFilePath(file), "utf8");
    return (
      source.includes("CookieConsent") ||
      source.includes("cookie-consent") ||
      source.includes("tracking-consent")
    );
  });
  const policyMentionsNonEssentialConsent =
    cookiePolicySource.includes("icke-nodvandig") &&
    cookiePolicySource.includes("samtycke") &&
    cookiePolicySource.includes("marknadsforingspixlar");
  const ok =
    policyMentionsNonEssentialConsent &&
    (findings.length === 0 || consentMechanismPresent);

  return {
    ok,
    details: ok
      ? findings.length === 0
        ? "No non-essential tracking providers are detected, and the cookie policy requires consent before analytics or marketing pixels are enabled."
        : `Detected ${findings.join(", ")}, with a consent mechanism present. Confirm opt-in behavior before launch.`
      : findings.length > 0
        ? `Non-essential tracking is present without a detected consent mechanism: ${findings.join(", ")}.`
        : "Cookie policy is missing the launch baseline for non-essential tracking consent.",
  };
}

export async function hasDisplayAssetReviewWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const { data, error } = await supabaseAdmin.rpc(
    "screenia_display_asset_review_workflow_ready",
  );
  ...
  return {
    ...
    details:
      data === true
        ? "Display material review notes, reviewed metadata, and constrained review statuses are ready."
        : "Display material review columns, status default/constraint, or existing statuses are not ready.",
  };
}

export async function hasCustomerPreviewDecisionWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  const accountRouteSource = readFileSync(
    projectFilePath("src/app/api/account/route.ts"),
  ...
  const [decisionResult, auditResult, notificationResult] = await Promise.all([
    supabaseAdmin
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Customers can approve previews or request changes from the account portal, with decision evidence, audit history, and admin notifications.",
  };
}

export async function hasRequestPrivacyConsentWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  const { data, error } = await supabaseAdmin.rpc(
    "screenia_request_privacy_consent_ready",
  ...
  return {
    ...
    details:
      data === true
        ? "Public request-form privacy acknowledgements are stored as current-version consent records before follow-up."
        : "Some public request records are missing current-version privacy_request consent evidence.",
  };
}

export async function hasPublicRequestIntakeWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const sourceIssues = [
    ...
    !requestRouteSource.includes("rateLimitHeaders")
    ...
    !requestRouteSource.includes("throwOnError: true")
      ? "public request route does not require and persist privacy consent before follow-up"
      : null,
    ...
    !requestRouteSource.includes("Request confirmation email failure notification was not stored")
      ...
      : null,
    !requestRouteSource.includes("validPlanCodes") ||
    !requestRouteSource.includes("isValidEmail")
      ? "public request route does not validate plan and email inputs"
      : null,
    !landingPageSource.includes("/api/onboarding-requests") ||
    !landingPageSource.includes("privacyAccepted")
      ? "landing page does not submit privacy-aware requests to the intake route"
      : null,
  ].filter(Boolean);
  ...
  const [customerResult, consentResult, auditResult, notificationResult] =
    await Promise.all([
      supabaseAdmin
        .from("customers")
        .select(
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Public customer requests are rate limited, honeypot protected, privacy-consented, audited, confirmed by email, and surfaced to admins.",
  };
}

export async function hasAdminNotificationWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const errors = [notificationResult.error, auditResult.error].filter(Boolean);

  return {
    ok: errors.length === 0,
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Admin notifications and audit events are available for audited acknowledgement actions, with reason-required bulk acknowledgement.",
  };
}

export async function hasOperationalFulfillmentReadiness(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const [customerResult, deviceResult, playlistResult, subscriptionResult, auditResult] =
    await Promise.all([
      supabaseAdmin
        .from("customers")
      ...
      supabaseAdmin
        ...
        .limit(1000),
      supabaseAdmin.from("playlists").select("id, device_id").limit(1000),
      supabaseAdmin
        .from("customer_subscriptions")
        .select(
          "id, customer_id, status, fulfillment_status, inventory_status, tracking_number, tracking_url",
        )
        .limit(1000),
      supabaseAdmin
        .from("audit_events")
        .select("id, event_type")
        .eq("event_type", "admin_order_operation_updated")
        .limit(1),
    ]);
  const errors = [
    customerResult.error,
  ...
  if (sourceIssues.length > 0 || errors.length > 0) {
    return {
      ok: false,
      details: [
        ...sourceIssues,
  ...
  const activeCustomerIds = new Set(
    (customerResult.data || [])
      .filter((customer) => {
        const status = String(customer.status || "");
        const paymentStatus = String(customer.payment_status || "");
        const accessStatus = String(customer.service_access_status || "");

        return (
          status === "active" ||
          paymentStatus === "paid" ||
          accessStatus === "active" ||
          accessStatus === "active_until_period_end"
        );
      })
      .map((customer) => customer.id),
  );
  const activeDeviceCustomerIds = new Set(
    devices
      .filter((device) => device.is_active)
      .map((device) => device.customer_id)
      .filter(Boolean),
  );
  const playlistDeviceIds = new Set(playlists.map((playlist) => playlist.device_id));
  const activeDevicesWithoutPlaylists = devices.filter(
    (device) => device.is_active && !playlistDeviceIds.has(device.id),
  ).length;
  const activeCustomersWithoutDevices = Array.from(activeCustomerIds).filter(
    (customerId) => !activeDeviceCustomerIds.has(customerId),
  ).length;
  const shippedWithoutTracking = (subscriptionResult.data || []).filter(
    (subscription) =>
      ["shipped", "completed"].includes(
        String(subscription.fulfillment_status || ""),
      ) &&
      !subscription.tracking_number &&
      !subscription.tracking_url,
  ).length;
  const fulfillmentIssues = [
    activeCustomersWithoutDevices > 0
      ? `${activeCustomersWithoutDevices} active/paid customer(s) have no active display device`
      : null,
    activeDevicesWithoutPlaylists > 0
      ? `${activeDevicesWithoutPlaylists} active display device(s) have no playlist`
      : null,
    shippedWithoutTracking > 0
      ? `${shippedWithoutTracking} shipped/completed order(s) have no tracking reference`
      : null,
  ].filter(Boolean);

  return {
    ok: fulfillmentIssues.length === 0,
    details:
      fulfillmentIssues.length === 0
        ? "Active/paid customers have active devices, active devices have playlists, and shipped/completed orders include tracking evidence."
        : fulfillmentIssues.join(" | "),
  };
}

export async function hasInventoryOperationsWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const auditResult = await supabaseAdmin
    ...
    .select("id, event_type, actor_type, metadata")
    .limit(1);
  const errors = [inventoryResult.error, eventResult.error, auditResult.error].filter(
    Boolean,
  );
...
export async function hasDeviceManagementWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const sourceIssues = [
    ...
    !deviceCreateRouteSource.includes("Device name is required")
      ...
      : null,
    !deviceUpdateRouteSource.includes("admin_device_created") &&
  ...
  const deviceResult = await supabaseAdmin
    .from("devices")
    .select("id, device_code, customer_id, is_active")
    .limit(1);
  const playlistResult = await supabaseAdmin
    .from("playlists")
    .select("id, device_id, video_id")
    .limit(1);
  const videoResult = await supabaseAdmin
    .from("videos")
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Device creation, edits, activation/deactivation, deletion, and MP4 playlist media changes are reason-required, audited, and routed through server-side operations.",
  };
}

export async function hasAdminCustomerDraftWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const errors = [customerResult.error, auditResult.error].filter(Boolean);

  return {
    ok: errors.length === 0,
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Admin customer draft creation authenticates admins, prevents duplicate emails, stores opt-in-safe drafts, and audits the manual creation reason.",
  };
}

export async function hasAdminCustomerProfileEditWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  const customerRouteSource = readFileSync(
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Admin customer profile edits validate organisation number, billing email, and postal code; require an admin reason; and audit changed fields.",
  };
}

export async function hasCustomerDataExportWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Customer data export source tables and audit storage are available.",
  };
}

export async function hasAccountingExportWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  const exportRouteSource = readFileSync(
  ...
  const sourceIssues = [
    ...
    !exportRouteSource.includes("Content-Disposition")
      ...
      : null,
    !exportRouteSource.includes("Cache-Control")
      ? "accounting export is missing no-store headers"
      : null,
    !exportRouteSource.includes("stripe_invoice_id")
      ? "accounting export is missing Stripe invoice identifiers"
      : null,
    !exportRouteSource.includes("tax_amount_sek")
      ? "accounting export is missing VAT amount fields"
      : null,
  ...
  const [subscriptionResult, customerResult, pricingResult, auditResult] =
    await Promise.all([
      supabaseAdmin
        .from("customer_subscriptions")
        .select(
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Admin accounting CSV export includes order, VAT, payment, customer, and Stripe invoice evidence and records audited downloads.",
  };
}

export async function hasVatSummaryWorkflow(supabaseAdmin: SupabaseClient) {
  ...
  const sourceIssues = [
    ...
    !vatRouteSource.includes("text/csv; charset=utf-8")
      ...
      : null,
    !vatRouteSource.includes("Cache-Control")
      ? "VAT summary is missing no-store headers"
      : null,
    !vatRouteSource.includes("tax_amount_sek")
      ? "VAT summary is missing VAT amount fields"
      : null,
  ...
  const [subscriptionResult, taxPaymentResult, auditResult] = await Promise.all([
    supabaseAdmin
      .from("customer_subscriptions")
      .select(
        "id, order_number, stripe_invoice_id, tax_amount_sek, total_amount_sek, stripe_payment_status",
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Admin VAT period summary can export paid invoice totals, included VAT, net amounts, and Stripe invoice references with audit history.",
  };
}

export async function hasTaxPaymentRegisterWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const sourceIssues = [
    ...
    !taxRouteSource.includes("admin_tax_payment_recorded")
      ? "tax payment records are not audited"
      : null,
    ...
    !taxRouteSource.includes("Tax payment audit failed, rollback failed, and urgent admin visibility could not be stored")
      ...
      : null,
    !taxUpdateRouteSource.includes("export async function PATCH")
      ? "tax payment records cannot be updated through an admin API"
      : null,
    !taxUpdateRouteSource.includes("admin_tax_payment_updated")
      ? "tax payment updates are not audited"
      : null,
  ...
  const [taxPaymentResult, auditResult] = await Promise.all([
    supabaseAdmin
      .from("tax_payments")
      .select(
  ...
  const errors = [taxPaymentResult.error, auditResult.error].filter(Boolean);

  return {
    ok: errors.length === 0,
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Admins can record VAT/tax periods with status, amount, payment evidence, reference, reason, and audit history.",
  };
}

export async function hasPrivacyIncidentWorkflow(supabaseAdmin: SupabaseClient) {
  ...
  const [incidentResult, auditResult, notificationResult] = await Promise.all([
    supabaseAdmin
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Admins can record privacy/security incidents, track status and notification decisions, notify admins, and audit every change.",
  };
}

export async function hasAdminAccessReviewWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const [accessResult, auditResult, notificationResult] = await Promise.all([
    supabaseAdmin
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Admins can record and update admin access reviews with MFA/access evidence, reason-required audit history, and notifications for risky access.",
  };
}

export async function hasBackupRestoreDrillWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const [drillResult, auditResult, notificationResult] = await Promise.all([
    supabaseAdmin
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Admins can record backup coverage and restore-test evidence with reason-required audit history and urgent notifications when recovery needs attention.",
  };
}

export async function hasDataRetentionReviewWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const [retentionResult, auditResult, notificationResult] = await Promise.all([
    supabaseAdmin
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Admins can record data retention decisions with legal basis, retention date, recommended action, reason-required audit history, and notifications for deletion/anonymization review.",
  };
}

export async function hasDataSubjectRequestWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  ...
  const [requestResult, auditResult, messageResult] = await Promise.all([
    supabaseAdmin
  ...
  return {
    ...
    details: errors.length
      ? errors.map((error) => error?.message).join(" | ")
      : "Customers can submit privacy requests that create deadline-tracked admin records, and admins can update those records with audit history.",
  };
}

export async function hasCustomerDeletionSafetyWorkflow(
  supabaseAdmin: SupabaseClient,
) {
  const customerRouteSource = readFileSync(
    projectFilePath("src/app/api/admin/customers/[customerId]/route.ts"),
  ...
  if (auditResult.error) {
    return {
      ok: false,
      details: auditResult.error.message,
    };
  }
  ...
  return {
    ...
    details:
      sourceIssues.length === 0
        ? "Customer anonymization and deletion require admin reasons, block permanent deletion for payment history, delete linked auth users during anonymization, and retain auditability."
        : sourceIssues.join(" | "),
  };
}

export function hasRequiredSecurityHeaders() {
...
export function hasServiceWorkerCacheSafety() {
  const serviceWorkerSource = readFileSync(projectFilePath("public/sw.js"), "utf8");
  const sourceIssues = [
    ...
    !serviceWorkerSource.includes("caches.delete")
      ? "service worker does not clear older cache names on activation"
      : null,
  ].filter(Boolean);
  ...
  return {
    ...
    details:
      sourceIssues.length === 0
        ? "Service worker caching is restricted to explicit static assets, excludes sensitive routes, respects no-store/private responses, and clears old caches."
        : sourceIssues.join(" | "),
  };
}

export function hasCsrfProtectionPolicy() {
  const proxySource = readFileSync(projectFilePath("src/proxy.ts"), "utf8");
  ...
  return {
    ...
    details: missing.length
      ...
      : "Unsafe cross-origin or missing-origin API requests are blocked; Stripe and Resend webhooks are exempt.",
  };
}

export async function getLiveCheckoutBlockers(supabaseAdmin: SupabaseClient) {
  const missingConfirmations = getMissingLivePaymentConfirmations().map(
    (item) => item.label,
  );
  const [
    legalDocuments,
    legalBeforePaymentWorkflow,
    subscriptionMigration,
    swedishRegistrationNumberValidation,
    pricingConfiguration,
    privateVideoBucket,
    privateSensitiveStorage,
    webhookEventLedger,
    optInConsentDefaults,
    displayAssetReviewWorkflow,
    customerPreviewDecisionWorkflow,
    requestPrivacyConsentWorkflow,
    publicRequestIntakeWorkflow,
    adminNotificationWorkflow,
    adminAccessReviewWorkflow,
    backupRestoreDrillWorkflow,
    dataRetentionReviewWorkflow,
    processorComplianceReviewWorkflow,
    resendDeliveryEventWorkflow,
    legalChangeNoticeWorkflow,
    refundAndProductionBoundaryWorkflow,
    customerSupportReplyWorkflow,
    customerSupportTicketIntakeWorkflow,
    operationalFulfillment,
    inventoryOperationsWorkflow,
    deviceManagementWorkflow,
    adminCustomerDraftWorkflow,
    adminCustomerProfileEditWorkflow,
    customerDataExportWorkflow,
    dataSubjectRequestWorkflow,
    billingPortalWorkflow,
    accountingExportWorkflow,
    vatSummaryWorkflow,
    taxPaymentRegisterWorkflow,
    privacyIncidentWorkflow,
    customerDeletionSafetyWorkflow,
  ] = await Promise.all([
    hasCurrentLegalDocuments(supabaseAdmin),
    hasLegalBeforePaymentWorkflow(supabaseAdmin),
    hasSubscriptionOperationsMigration(supabaseAdmin),
    hasSwedishRegistrationNumberValidation(supabaseAdmin),
    hasPricingConfigurationReadiness(supabaseAdmin),
    hasPrivateVideoBucket(supabaseAdmin),
    hasPrivateSensitiveStorageBuckets(supabaseAdmin),
    hasStripeWebhookEventLedger(supabaseAdmin),
    hasOptInConsentDefaults(supabaseAdmin),
    hasDisplayAssetReviewWorkflow(supabaseAdmin),
    hasCustomerPreviewDecisionWorkflow(supabaseAdmin),
    hasRequestPrivacyConsentWorkflow(supabaseAdmin),
    hasPublicRequestIntakeWorkflow(supabaseAdmin),
    hasAdminNotificationWorkflow(supabaseAdmin),
    hasAdminAccessReviewWorkflow(supabaseAdmin),
    hasBackupRestoreDrillWorkflow(supabaseAdmin),
    hasDataRetentionReviewWorkflow(supabaseAdmin),
    hasProcessorComplianceReviewWorkflow(supabaseAdmin),
    hasResendDeliveryEventWorkflow(supabaseAdmin),
    hasLegalChangeNoticeWorkflow(supabaseAdmin),
    hasRefundAndProductionBoundaryWorkflow(supabaseAdmin),
    hasCustomerSupportReplyWorkflow(),
    hasCustomerSupportTicketIntakeWorkflow(supabaseAdmin),
    hasOperationalFulfillmentReadiness(supabaseAdmin),
    hasInventoryOperationsWorkflow(supabaseAdmin),
    hasDeviceManagementWorkflow(supabaseAdmin),
    hasAdminCustomerDraftWorkflow(supabaseAdmin),
    hasAdminCustomerProfileEditWorkflow(supabaseAdmin),
    hasCustomerDataExportWorkflow(supabaseAdmin),
    hasDataSubjectRequestWorkflow(supabaseAdmin),
    hasBillingPortalWorkflow(),
    hasAccountingExportWorkflow(supabaseAdmin),
    hasVatSummaryWorkflow(supabaseAdmin),
    hasTaxPaymentRegisterWorkflow(supabaseAdmin),
    hasPrivacyIncidentWorkflow(supabaseAdmin),
    hasCustomerDeletionSafetyWorkflow(supabaseAdmin),
  ]);
  ...
  const applicationUrl = getApplicationUrlReadiness();
  const companyIdentity = getCompanyIdentityReadiness();
  const finalLegalVersions = getFinalLegalVersionReadiness();
  ...
  const blockers: string[] = [...missingConfirmations];

  if (!applicationUrl.ok) {
    blockers.push(`Application URL (${applicationUrl.details})`);
  }

  if (!companyIdentity.ok) {
    blockers.push(`Company identity (${companyIdentity.details})`);
  }

  if (!finalLegalVersions.ok) {
    blockers.push(`Final legal versions (${finalLegalVersions.details})`);
  }

  if (!legalDocuments.ok) {
    blockers.push(`Legal documents (${legalDocuments.details})`);
  }

  if (!legalBeforePaymentWorkflow.ok) {
    blockers.push(
      `Legal before payment workflow (${legalBeforePaymentWorkflow.details})`,
    );
  }

  if (!subscriptionMigration.ok) {
    blockers.push(
      `Subscription entitlement migration (${subscriptionMigration.details})`,
    );
  }

  if (!swedishRegistrationNumberValidation.ok) {
    blockers.push(
      `Swedish organisation-number validation (${swedishRegistrationNumberValidation.details})`,
    );
  }

  if (!pricingConfiguration.ok) {
    blockers.push(`Pricing configuration (${pricingConfiguration.details})`);
  }

  if (!privateVideoBucket.ok) {
    blockers.push(`Private display video bucket (${privateVideoBucket.details})`);
  }

  if (!privateSensitiveStorage.ok) {
    blockers.push(`Private sensitive storage (${privateSensitiveStorage.details})`);
  }

  if (!webhookEventLedger.ok) {
    blockers.push(`Stripe webhook event ledger (${webhookEventLedger.details})`);
  }

  if (!financialRiskWebhooks.ok) {
    blockers.push(
      `Stripe dispute and refund webhooks (${financialRiskWebhooks.details})`,
    );
  }

  if (!optInConsentDefaults.ok) {
    blockers.push(`Opt-in consent defaults (${optInConsentDefaults.details})`);
  }

  if (!cookieTrackingGovernance.ok) {
    blockers.push(
      `Cookie and tracking governance (${cookieTrackingGovernance.details})`,
    );
  }

  if (!displayAssetReviewWorkflow.ok) {
    blockers.push(
      `Display material review workflow (${displayAssetReviewWorkflow.details})`,
    );
  }

  if (!customerPreviewDecisionWorkflow.ok) {
    blockers.push(
      `Customer preview decision workflow (${customerPreviewDecisionWorkflow.details})`,
    );
  }

  if (!requestPrivacyConsentWorkflow.ok) {
    blockers.push(
      `Public request privacy consent (${requestPrivacyConsentWorkflow.details})`,
    );
  }

  if (!publicRequestIntakeWorkflow.ok) {
    blockers.push(
      `Public request intake workflow (${publicRequestIntakeWorkflow.details})`,
    );
  }

  if (!adminNotificationWorkflow.ok) {
    blockers.push(
      `Admin notification workflow (${adminNotificationWorkflow.details})`,
    );
  }

  if (!adminAccessReviewWorkflow.ok) {
    blockers.push(
      `Admin access review workflow (${adminAccessReviewWorkflow.details})`,
    );
  }

  if (!backupRestoreDrillWorkflow.ok) {
    blockers.push(
      `Backup restore drill workflow (${backupRestoreDrillWorkflow.details})`,
    );
  }

  if (!dataRetentionReviewWorkflow.ok) {
    blockers.push(
      `Data retention review workflow (${dataRetentionReviewWorkflow.details})`,
    );
  }

  if (!processorComplianceReviewWorkflow.ok) {
    blockers.push(
      `Processor compliance review workflow (${processorComplianceReviewWorkflow.details})`,
    );
  }

  if (!resendDeliveryEventWorkflow.ok) {
    blockers.push(
      `Resend delivery event workflow (${resendDeliveryEventWorkflow.details})`,
    );
  }

  if (!legalChangeNoticeWorkflow.ok) {
    blockers.push(
      `Legal change notice workflow (${legalChangeNoticeWorkflow.details})`,
    );
  }

  if (!refundAndProductionBoundaryWorkflow.ok) {
    blockers.push(
      `Refund and production boundary workflow (${refundAndProductionBoundaryWorkflow.details})`,
    );
  }

  if (!customerSupportReplyWorkflow.ok) {
    blockers.push(
      `Customer support reply workflow (${customerSupportReplyWorkflow.details})`,
    );
  }

  if (!customerSupportTicketIntakeWorkflow.ok) {
    blockers.push(
      `Customer support ticket intake workflow (${customerSupportTicketIntakeWorkflow.details})`,
    );
  }

  if (!operationalFulfillment.ok) {
    blockers.push(
      `Operational fulfillment readiness (${operationalFulfillment.details})`,
    );
  }
  ...
  if (!deviceManagementWorkflow.ok) {
    blockers.push(
      `Device management workflow (${deviceManagementWorkflow.details})`,
    );
  }

  if (!adminCustomerDraftWorkflow.ok) {
    blockers.push(
      `Admin customer draft workflow (${adminCustomerDraftWorkflow.details})`,
    );
  }

  if (!adminCustomerProfileEditWorkflow.ok) {
    blockers.push(
      `Admin customer profile edit workflow (${adminCustomerProfileEditWorkflow.details})`,
    );
  }

  if (!customerDataExportWorkflow.ok) {
    blockers.push(
      `Customer data export workflow (${customerDataExportWorkflow.details})`,
    );
  }

  if (!dataSubjectRequestWorkflow.ok) {
    blockers.push(
      `Data subject request workflow (${dataSubjectRequestWorkflow.details})`,
    );
  }

  if (!billingPortalWorkflow.ok) {
    blockers.push(`Billing portal workflow (${billingPortalWorkflow.details})`);
  }

  if (!accountingExportWorkflow.ok) {
    blockers.push(
      `Accounting export workflow (${accountingExportWorkflow.details})`,
    );
  }

  if (!vatSummaryWorkflow.ok) {
    blockers.push(`VAT summary workflow (${vatSummaryWorkflow.details})`);
  }

  if (!taxPaymentRegisterWorkflow.ok) {
    blockers.push(
      `Tax payment register workflow (${taxPaymentRegisterWorkflow.details})`,
    );
  }

  if (!privacyIncidentWorkflow.ok) {
    blockers.push(
      `Privacy incident workflow (${privacyIncidentWorkflow.details})`,
    );
  }

  if (!customerDeletionSafetyWorkflow.ok) {
    blockers.push(
      `Customer deletion safety workflow (${customerDeletionSafetyWorkflow.details})`,
    );
  }

  if (!securityHeaders.ok) {
    blockers.push(`Security headers (${securityHeaders.details})`);
  }

  if (!sensitiveNoStore.ok) {
    blockers.push(`Sensitive route no-store policy (${sensitiveNoStore.details})`);
  }

  if (!serviceWorkerCacheSafety.ok) {
    blockers.push(
      `Service worker cache safety (${serviceWorkerCacheSafety.details})`,
    );
  }

  if (!csrfProtection.ok) {
    blockers.push(`CSRF protection policy (${csrfProtection.details})`);
  }

  if (!textQualityCheck.ok) {
    blockers.push(`Text quality check (${textQualityCheck.details})`);
  }

  if (!emailReadiness.productionSafe) {
    blockers.push(`Production email sender (${emailReadiness.details})`);
  }

  if (!transactionalEmailWorkflow.ok) {
    blockers.push(
      `Transactional email workflow (${transactionalEmailWorkflow.details})`,
    );
  }

  if (!passwordPolicy.ok) {
    blockers.push(`Customer password policy (${passwordPolicy.details})`);
  }

  if (!passwordResetProtection.ok) {
    blockers.push(
      `Password reset abuse protection (${passwordResetProtection.details})`,
    );
  }

  if (!loginAttemptProtection.ok) {
    blockers.push(`Login attempt protection (${loginAttemptProtection.details})`);
  }
