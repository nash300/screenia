...
const invoiceId = process.argv[3]?.trim();
const targetUrl = process.argv[4]?.trim() || "http://localhost:3000/api/stripe/webhook";

if (!eventType || !invoiceId) {
  throw new Error(
    "Usage: node scripts/replay-stripe-invoice-event.mjs <event-type> <invoice-id> [localhost-webhook-url]",
  );
}

if (!/^invoice\.(paid|payment_failed)$/.test(eventType)) {
  throw new Error("Only invoice.paid and invoice.payment_failed test events are supported.");
}
...
for await (const event of stripe.events.list({ type: eventType, limit: 100 })) {
  if (event.data?.object?.id === invoiceId) {
    sourceEvent = event;
    break;
  }
}
...
if (!sourceEvent) {
  throw new Error(`No ${eventType} event was found for invoice ${invoiceId}.`);
}

const replayEvent = {
  ...sourceEvent,
  id: `evt_screenia_local_replay_${Date.now()}`,
