...
const disputeId = process.argv[3]?.trim();
const targetUrl = process.argv[4]?.trim() || "http://localhost:3000/api/stripe/webhook";

if (!eventType || !disputeId) {
  throw new Error(
    "Usage: node scripts/replay-stripe-dispute-event.mjs <event-type> <dispute-id> [localhost-webhook-url]",
  );
}

if (!/^charge\.dispute\.(created|updated|closed)$/.test(eventType)) {
  throw new Error(
    "Only charge.dispute.created, charge.dispute.updated, and charge.dispute.closed test events are supported.",
  );
}
...
for await (const event of stripe.events.list({ type: eventType, limit: 100 })) {
  if (event.data?.object?.id === disputeId) {
    sourceEvent = event;
    break;
  }
}
...
if (!sourceEvent) {
  throw new Error(`No ${eventType} event was found for dispute ${disputeId}.`);
}

const replayEvent = {
  ...sourceEvent,
  id: `evt_screenia_local_dispute_replay_${Date.now()}`,
