...
const { data: plans, error: plansError } = await supabase
  .from("pricing_plans")
  .select(
    "id,code,name,resolution,setup_fee_sek,setup_included_screens,additional_setup_fee_sek,tax_behavior,stripe_setup_price_id,stripe_additional_setup_price_id",
  )
  .eq("is_active", true)
  .order("code");
...
const { error: updateError } = await supabase
  .from("pricing_plans")
  .update({ stripe_additional_setup_price_id: sharedPriceId })
  .in(
    "id",
...
console.log(
  `Synced one shared Stripe test price (${sharedPriceId}) for 249 SEK additional-screen setup across ${plans.length} active plans.`,
);
